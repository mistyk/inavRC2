...
# 31 "./src/main/navigation/navigation.h"
...
const motorConfig_t pgResetTemplate_motorConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .minthrottle = 1150, .motorPwmProtocol = PWM_TYPE_STANDARD, .motorPwmRate = 400, .maxthrottle = 1850, .mincommand = 1000, .motorAccelTimeMs = 0, .motorDecelTimeMs = 0, .digitalIdleOffsetValue = 450 }
# 105 "./src/main/flight/mixer.c"
 ;

static motorMixer_t currentMixer[12];

motorMixer_t customMotorMixer_SystemArray[12]; motorMixer_t customMotorMixer_CopyArray[12]; extern const pgRegistry_t customMotorMixer_Registry; const pgRegistry_t customMotorMixer_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 4 | (0 << 12), .size = (sizeof(motorMixer_t) * 12) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&customMotorMixer_SystemArray, .copy = (uint8_t*)&customMotorMixer_CopyArray, .ptr = 0, .reset = {.ptr = 0}, };
...
# 121 "./src/main/flight/mixer.c" 3 4
_Bool 
# 121 "./src/main/flight/mixer.c"
    mixerIsOutputSaturated(void)
{
...
void writeMotors(void)
{
    for (int i = 0; i < motorCount; i++) {
        ...
        if (isMotorProtocolDshot()) {
            const float dshotMinThrottleOffset = (2047 - 48) / 10000.0f * motorConfig()->digitalIdleOffsetValue;

            if (feature(FEATURE_3D)) {
                if (motor[i] >= motorConfig()->minthrottle && motor[i] <= flight3DConfig()->deadband3d_low) {
                    motorValue = scaleRangef(motor[i], motorConfig()->minthrottle, flight3DConfig()->deadband3d_low, 1047, dshotMinThrottleOffset + 48);
                    motorValue = constrain(motorValue, 48, 1047);
                }
                else if (motor[i] >= flight3DConfig()->deadband3d_high && motor[i] <= motorConfig()->maxthrottle) {
                    motorValue = scaleRangef(motor[i], flight3DConfig()->deadband3d_high, motorConfig()->maxthrottle, dshotMinThrottleOffset + 1048, 2047);
                    motorValue = constrain(motorValue, 1048, 2047);
                }
                else {
                    motorValue = 0;
                }
                ...
                if (motor[i] < motorConfig()->minthrottle) {
                    ...
                    motorValue = scaleRangef(motor[i], motorConfig()->minthrottle, motorConfig()->maxthrottle, (dshotMinThrottleOffset + 48), 2047);
                    motorValue = constrain(motorValue, (dshotMinThrottleOffset + 48), 2047);
                }
            }
        }
        else {
...
{
    static float motorPrevious[12] = { 0 };
