...
# 9 "/usr/arm-none-eabi/include/xlocale.h" 3
struct __locale_t;
...
# 47 "./src/main/sensors/battery.c" 2
...
# 51 "./src/main/sensors/battery.c" 2
...
# 53 "./src/main/sensors/battery.c" 2
...
# 55 "./src/main/sensors/battery.c" 2
...
# 57 "./src/main/sensors/battery.c" 2


#define ADCVREF 3300

#define VBATT_CELL_FULL_MAX_DIFF 10
#define VBATT_PRESENT_THRESHOLD 100
...
static 
# 76 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 76 "./src/main/sensors/battery.c"
...
# 76 "./src/main/sensors/battery.c"
                                               ;
static 
# 77 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 77 "./src/main/sensors/battery.c"
...
# 77 "./src/main/sensors/battery.c"
                                           ;
static 
# 78 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 78 "./src/main/sensors/battery.c"
...
static uint16_t powerSupplyImpedance = 0;
static uint16_t sagCompensatedVBat = 0;
static 
# 83 "./src/main/sensors/battery.c" 3 4
      _Bool 
# 83 "./src/main/sensors/battery.c"
           powerSupplyImpedanceIsValid = 
# 83 "./src/main/sensors/battery.c" 3 4
                                         0
# 83 "./src/main/sensors/battery.c"
                                              ;

static int16_t amperage = 0;
static int32_t power = 0;
static int32_t mAhDrawn = 0;
static int32_t mWhDrawn = 0;

batteryState_e batteryState;
const batteryProfile_t *currentBatteryProfile;

extern void pgResetFn_batteryProfiles(batteryProfile_t *); batteryProfile_t batteryProfiles_SystemArray[3]; batteryProfile_t batteryProfiles_CopyArray[3]; extern const pgRegistry_t batteryProfiles_Registry; const pgRegistry_t batteryProfiles_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 11 | (0 << 12), .size = (sizeof(batteryProfile_t) * 3) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&batteryProfiles_SystemArray, .copy = (uint8_t*)&batteryProfiles_CopyArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_batteryProfiles}, };

void pgResetFn_batteryProfiles(batteryProfile_t *instance)
{
    for (int i = 0; i < 3; i++) {
        static const batteryProfile_t _reset_template_114 = { .cells = 0, .voltage = { .cellDetect = 430, .cellMax = 420, .cellMin = 330, .cellWarning = 350 }, .capacity = { .value = 0, .warning = 0, .critical = 0, .unit = BAT_CAPACITY_UNIT_MAH, } }; memcpy((&instance[i]), &_reset_template_114, sizeof(*(&instance[i])));
# 114 "./src/main/sensors/battery.c"
         ;
    }
}

extern const batteryMetersConfig_t pgResetTemplate_batteryMetersConfig; batteryMetersConfig_t batteryMetersConfig_System; batteryMetersConfig_t batteryMetersConfig_Copy; extern const pgRegistry_t batteryMetersConfig_Registry; const pgRegistry_t batteryMetersConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 45 | (0 << 12), .size = sizeof(batteryMetersConfig_t) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&batteryMetersConfig_System, .copy = (uint8_t*)&batteryMetersConfig_Copy, .ptr = 0, .reset = {.ptr = (void*)&pgResetTemplate_batteryMetersConfig}, };

const batteryMetersConfig_t pgResetTemplate_batteryMetersConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .voltage_scale = 1100, .current = { .type = CURRENT_SENSOR_ADC, .scale = 317, .offset = 0 }, .voltageSource = BAT_VOLTAGE_RAW, .cruise_power = 0, .idle_power = 0, .rth_energy_margin = 5, .throttle_compensation_weight = 1.0f }
# 138 "./src/main/sensors/battery.c"
 ;
...
static int8_t profileDetect() {
    profile_comp_t profile_comp_array[3];
    ...
    qsort(profile_comp_array, 3, sizeof(*profile_comp_array), (int (*)(const void *, const void *))profile_compare);
...
# 201 "./src/main/sensors/battery.c" 3 4
                                                    _Bool 
# 201 "./src/main/sensors/battery.c"
                                                         justConnected)
{
    ...
    vbat = (uint64_t)adcGetChannel(ADC_BATTERY) * batteryMetersConfig()->voltage_scale * 3300 / (0xFFF * 1000);

    if (justConnected) {
        pt1FilterReset(&vbatFilterState, vbat);
    } else {
        vbat = pt1FilterApply4(&vbatFilterState, vbat, 1, timeDelta * 1e-6f);
    }
}
...
void batteryUpdate(timeUs_t timeDelta)
{
    ...
    if (batteryState == BATTERY_NOT_PRESENT && vbat > 100) {
...
# 228 "./src/main/sensors/battery.c" 3 4
                                       1
# 228 "./src/main/sensors/battery.c"
                                           );
        ...
        if (detectedProfileIndex != -1) {
            ...
            if (batteryCellCount > 8) batteryCellCount = 8;
        }
        ...
        batteryFullWhenPluggedIn = vbat >= (batteryFullVoltage - batteryCellCount * 10);
        ...
        updateBatteryVoltage(timeDelta, 
# 254 "./src/main/sensors/battery.c" 3 4
                                       0
# 254 "./src/main/sensors/battery.c"
                                            );
    ...
    if (batteryState != BATTERY_NOT_PRESENT) {
        ...
        if (batteryUseCapacityThresholds) {
            ...
            {
                case BATTERY_OK:
                    if (stateVoltage <= (batteryWarningVoltage - 10))
                        batteryState = BATTERY_WARNING;
                    break;
                case BATTERY_WARNING:
                    if (stateVoltage <= (batteryCriticalVoltage - 10)) {
                        batteryState = BATTERY_CRITICAL;
                    } else if (stateVoltage > (batteryWarningVoltage + 10)){
                        batteryState = BATTERY_OK;
                    }
                    break;
        ...
        if ((armingFlags & (ARMED)) || !(armingFlags & (WAS_EVER_ARMED)) || failsafeIsActive())
            switch (batteryState) {
                case BATTERY_WARNING:
...
# 322 "./src/main/sensors/battery.c" 3 4
_Bool 
# 322 "./src/main/sensors/battery.c"
    batteryWasFullWhenPluggedIn(void)
{
...
# 327 "./src/main/sensors/battery.c" 3 4
_Bool 
# 327 "./src/main/sensors/battery.c"
    batteryUsesCapacityThresholds(void)
{
...
# 332 "./src/main/sensors/battery.c" 3 4
_Bool 
# 332 "./src/main/sensors/battery.c"
    isBatteryVoltageConfigured(void)
{
...
# 400 "./src/main/sensors/battery.c" 3 4
_Bool 
# 400 "./src/main/sensors/battery.c"
    isAmperageConfigured(void)
{
...
void currentMeterUpdate(timeUs_t timeDelta)
{
    ...
    switch (batteryMetersConfig()->current.type) {
        case CURRENT_SENSOR_ADC:
            {
                int32_t microvolts = ((uint32_t)adcGetChannel(ADC_CURRENT) * 3300 * 100) / 0xFFF * 10 - (int32_t)batteryMetersConfig()->current.offset * 100;
                amperage = microvolts / batteryMetersConfig()->current.scale;
                amperage = pt1FilterApply4(&amperageFilterState, amperage, 1, timeDelta * 1e-6f);
                break;
            }
...
void powerMeterUpdate(timeUs_t timeDelta)
{
    ...
    power = (int32_t)amperage * vbat / 100;
    int32_t heatLossesCompensatedPower_mW = (int32_t)amperage * vbat / 10 + (((int64_t)amperage)*((int64_t)amperage)) * powerSupplyImpedance / 10000;
...
int32_t heatLossesCompensatedPower(int32_t power)
{
    return power + ((power * 100 / batteryWarningVoltage)*(power * 100 / batteryWarningVoltage)) * powerSupplyImpedance / 100000;
}
...
void sagCompensatedVBatUpdate(timeUs_t currentTime, timeUs_t timeDelta)
{
    ...
    if (batteryState == BATTERY_NOT_PRESENT) {
        ...
        powerSupplyImpedanceIsValid = 
# 501 "./src/main/sensors/battery.c" 3 4
                                     0
# 501 "./src/main/sensors/battery.c"
                                          ;
        ...
        if (!recordTimestamp) {
            ...
            if (impedanceSampleCount <= 10) {
                impedanceSampleCount += 1;
            }

            if (impedanceFilterState.state) {
                pt1FilterSetTimeConstant(&impedanceFilterState, impedanceSampleCount > 10 ? 1.2 : 0.5);
            ...
            if (impedanceSampleCount > 10) {
                ...
                powerSupplyImpedanceIsValid = 
# 530 "./src/main/sensors/battery.c" 3 4
                                             1
# 530 "./src/main/sensors/battery.c"
                                                 ;
            }

        }

        uint16_t sagCompensatedVBatSample = ( __extension__ ({ __typeof__(batteryFullVoltage) _left0 = (batteryFullVoltage); __typeof__(vbat + (int32_t)powerSupplyImpedance * amperage / 1000) _right1 = (vbat + (int32_t)powerSupplyImpedance * amperage / 1000); _left0 < _right1 ? _left0 : _right1; }));
    ...
    {if (debugMode == (DEBUG_SAG_COMP_VOLTAGE)) {debug[(0)] = (powerSupplyImpedance);}};
    {if (debugMode == (DEBUG_SAG_COMP_VOLTAGE)) {debug[(1)] = (sagCompensatedVBat);}};
...
void batteryDisableProfileAutoswitch(void) {
    profileAutoswitchDisable = 
# 559 "./src/main/sensors/battery.c" 3 4
                              1
# 559 "./src/main/sensors/battery.c"
                                  ;
}


# 562 "./src/main/sensors/battery.c" 3 4
_Bool 
# 562 "./src/main/sensors/battery.c"
    isPowerSupplyImpedanceValid(void) {
