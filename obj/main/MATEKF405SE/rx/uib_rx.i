...
# 31 "./src/main/uav_interconnect/uav_interconnect.h" 2
...
static uint8_t rxUIBFrameStatus(rxRuntimeConfig_t *rxRuntimeConfig)
{
    ...
    if (!uibDetectAndActivateDevice(0x80)) {
        return RX_FRAME_FAILSAFE;
    }

    if (uibGetUnansweredRequests(0x80) > 10) {
        return RX_FRAME_FAILSAFE;
    }

    if (uibDataAvailable(0x80)) {
        rcUibReceiverData_t uibDataTmp;
        uibRead(0x80, (uint8_t*)&uibDataTmp, sizeof(uibDataTmp));
