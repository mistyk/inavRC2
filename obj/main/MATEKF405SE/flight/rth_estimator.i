# 1 "./src/main/flight/rth_estimator.c"
...
# 3 "./src/main/flight/rth_estimator.c" 2
...
# 5 "./src/main/flight/rth_estimator.c" 2
...
# 6 "./src/main/flight/rth_estimator.c" 2
...
# 8 "./src/main/flight/rth_estimator.c" 2
...
# 44 "./src/main/fc/fc_core.h"
                  bootLoader);
# 9 "./src/main/flight/rth_estimator.c" 2
...
# 10 "./src/main/flight/rth_estimator.c" 2
...
void imuInit(void);
# 12 "./src/main/flight/rth_estimator.c" 2
...
# 13 "./src/main/flight/rth_estimator.c" 2
...
# 14 "./src/main/flight/rth_estimator.c" 2
...
# 16 "./src/main/flight/rth_estimator.c" 2
...
# 18 "./src/main/flight/rth_estimator.c" 2
# 31 "./src/main/flight/rth_estimator.c"
static float windDriftCompensationAngle(float forwardSpeed, float heading, float horizontalWindSpeed, float windHeading) {
    return (((3.14159265358979323846f / 2 - acos_approx(-horizontalWindSpeed * sin_approx(((windHeading - heading) * (3.14159265358979323846f / 180.0f))) / forwardSpeed))) / (3.14159265358979323846f / 180.0f));
}
# 43 "./src/main/flight/rth_estimator.c"
static float windDriftCorrectedForwardSpeed(float forwardSpeed, float heading, float horizontalWindSpeed, float windHeading) {
    return forwardSpeed * cos_approx(((windDriftCompensationAngle(forwardSpeed, heading, horizontalWindSpeed, windHeading)) * (3.14159265358979323846f / 180.0f)));
}
# 54 "./src/main/flight/rth_estimator.c"
static float forwardWindSpeed(float heading, float horizontalWindSpeed, float windHeading) {
    return horizontalWindSpeed * cos_approx(((windHeading - heading) * (3.14159265358979323846f / 180.0f)));
}
# 66 "./src/main/flight/rth_estimator.c"
...
static float estimateRTHAltitudeChangeTime(float altitudeChange, float verticalWindSpeed) {
    ...
    const float estimatedVerticalSpeed = (float)navConfig()->fw.cruise_speed / 100 * sin_approx(((RTHAltitudeChangePitchAngle(altitudeChange)) * (3.14159265358979323846f / 180.0f))) + verticalWindSpeed;
...
static float estimateRTHAltitudeChangeGroundDistance(float altitudeChange, float horizontalWindSpeed, float windHeading, float verticalWindSpeed) {
    ...
    float estimatedHorizontalSpeed = (float)navConfig()->fw.cruise_speed / 100 * cos_approx(((RTHAltitudeChangePitchAngle(altitudeChange)) * (3.14159265358979323846f / 180.0f))) + forwardWindSpeed((((float)attitude.values.yaw) / 10), horizontalWindSpeed, windHeading);
...
static float estimateRTHDistanceAndHeadingAfterAltitudeChange(float altitudeChange, float horizontalWindSpeed, float windHeading, float verticalWindSpeed, float *heading) {
    ...
    if (navConfig()->general.flags.rth_climb_first && (altitudeChange > 0)) {
        float headingDiff = (((((float)attitude.values.yaw) / 10) - GPS_directionToHome) * (3.14159265358979323846f / 180.0f));
        ...
        const float reverseHeadingDiff = ((atan2_approx(triangleAltitude, triangleAltitudeToReturnStart)) / (3.14159265358979323846f / 180.0f));
        *heading = ((wrap_36000(((180 + reverseHeadingDiff + (((float)attitude.values.yaw) / 10)) * 100))) / 100);
        return sqrt(((triangleAltitude)*(triangleAltitude)) + ((triangleAltitudeToReturnStart)*(triangleAltitudeToReturnStart)));
    } else {
...
# 132 "./src/main/flight/rth_estimator.c"
    ...
    const float horizontalWindSpeed = takeWindIntoAccount ? getEstimatedHorizontalWindSpeed(&windHeading) / 100 : 0;
    const float windHeadingDegrees = (((float)windHeading) / 100);
    ...
    const float RTH_speed = windCompensatedForwardSpeed((float)navConfig()->fw.cruise_speed / 100, ((attitude.values.yaw) / 10), horizontalWindSpeed, windHeadingDegrees);

    {if (debugMode == (DEBUG_REM_FLIGHT_TIME)) {debug[(0)] = (lrintf(RTH_altitude_change * 100));}};
    {if (debugMode == (DEBUG_REM_FLIGHT_TIME)) {debug[(1)] = (lrintf(RTH_distance * 100));}};
    {if (debugMode == (DEBUG_REM_FLIGHT_TIME)) {debug[(2)] = (lrintf(RTH_speed * 100));}};
    {if (debugMode == (DEBUG_REM_FLIGHT_TIME)) {debug[(3)] = (lrintf(horizontalWindSpeed * 100));}};

    if (RTH_speed <= 0)
        return -2;

    const uint32_t time_to_home = RTH_distance / RTH_speed;
    const uint32_t energy_to_home = estimateRTHAltitudeChangeEnergy(RTH_altitude_change, verticalWindSpeed) * 1000 + heatLossesCompensatedPower(batteryMetersConfig()->idle_power + batteryMetersConfig()->cruise_power) * time_to_home / 360;
    const uint32_t energy_margin_abs = (currentBatteryProfile->capacity.value - currentBatteryProfile->capacity.critical) * batteryMetersConfig()->rth_energy_margin / 100;
    const int32_t remaining_energy_before_rth = getBatteryRemainingCapacity() - energy_margin_abs - energy_to_home;

    if (remaining_energy_before_rth < 0)
        return 0;
...
# 181 "./src/main/flight/rth_estimator.c" 3 4
                                             _Bool 
# 181 "./src/main/flight/rth_estimator.c"
                                                  takeWindIntoAccount) {
...
# 203 "./src/main/flight/rth_estimator.c" 3 4
                                           _Bool 
# 203 "./src/main/flight/rth_estimator.c"
                                                takeWindIntoAccount) {
