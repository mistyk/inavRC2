...
# 56 "./src/main/drivers/max7456.h" 3 4
_Bool 
# 56 "./src/main/drivers/max7456.h"
    max7456ReadChar(uint8_t x, uint8_t y, uint16_t *c, uint8_t *mode);
...
#define SYM_THR 0x04
#define SYM_AH_DECORATION_UP 0x05
...
#define SYM_AH_DECORATION_MIN 0x10
#define SYM_AH_DECORATION 0x13
#define SYM_AH_DECORATION_MAX 0x15
#define SYM_AH_DECORATION_COUNT (SYM_AH_DECORATION_MAX - SYM_AH_DECORATION_MIN + 1)

#define SYM_WIND_HORIZONTAL 0x16
#define SYM_WIND_VERTICAL 0x17
...
#define SYM_AH_DECORATION_DOWN 0x24



#define SYM_AH_CH_LEFT 0x26
#define SYM_AH_CH_RIGHT 0x27
...
#define SYM_MAH_KM_0 0x9D
#define SYM_MAH_KM_1 0x9E
#define SYM_MS 0x9F
...
#define SYM_ALT 0xAA
#define SYM_WH 0xAB
#define SYM_WH_KM_0 0xAC
#define SYM_WH_KM_1 0xAD
#define SYM_WATT 0xAE
#define SYM_SCALE 0xAF
#define SYM_MPH 0xB0
#define SYM_ALT_M 0xB1
#define SYM_ALT_KM 0xB2
#define SYM_ALT_FT 0xB3
#define SYM_ALT_KFT 0xB4
#define SYM_DIST_M 0xB5
#define SYM_DIST_KM 0xB6
#define SYM_DIST_FT 0xB7
#define SYM_DIST_MI 0xB8
#define SYM_M 0xB9
#define SYM_KM 0xBA
#define SYM_MI 0xBB
...
#define SYM_AUTO_THR0 0xCA
#define SYM_AUTO_THR1 0xCB
...
#define SYM_AH_CH_AIRCRAFT0 0xDA
#define SYM_AH_CH_AIRCRAFT1 0xDB
#define SYM_AH_CH_AIRCRAFT2 0xDC
#define SYM_AH_CH_AIRCRAFT3 0xDD
#define SYM_AH_CH_AIRCRAFT4 0xDE
...
# 200 "./src/main/drivers/max7456.c" 3 4
   _Bool 
# 200 "./src/main/drivers/max7456.c"
        isInitialized;
    
# 201 "./src/main/drivers/max7456.c" 3 4
   _Bool 
# 201 "./src/main/drivers/max7456.c"
        mutex;
...
static 
# 207 "./src/main/drivers/max7456.c" 3 4
      _Bool 
# 207 "./src/main/drivers/max7456.c"
           max7456ReadVM0(uint8_t *vm0)
{
    return busRead(state.dev, 0x00 | 0x80, vm0);
}
           ...
           max7456IsBusy(void)
{
    ...
    if (busRead(state.dev, 0xA0, &status)) {
        return status & 0x20;
    }

    return 
# 220 "./src/main/drivers/max7456.c" 3 4
          1
# 220 "./src/main/drivers/max7456.c"
              ;
}
...
static 
# 240 "./src/main/drivers/max7456.c" 3 4
      _Bool 
# 240 "./src/main/drivers/max7456.c"
           max7456OSDSetEnabled(
# 240 "./src/main/drivers/max7456.c" 3 4
                                _Bool 
# 240 "./src/main/drivers/max7456.c"
                                     enabled)
{
    if (enabled && !(state.registers.vm0 | 0x08)) {
        state.registers.vm0 |= 0x08;
    } else if (!enabled && (state.registers.vm0 | 0x08)) {
        state.registers.vm0 &= ~0x08;
    } else {

        return 
# 248 "./src/main/drivers/max7456.c" 3 4
              0
# 248 "./src/main/drivers/max7456.c"
                   ;
    }
    return busWrite(state.dev, 0x00, state.registers.vm0);
}
...
# 253 "./src/main/drivers/max7456.c"
           max7456OSDIsEnabled(void)
{
    return state.registers.vm0 & 0x08;
}
...
static void max7456Lock(void)
{
    ...
    state.mutex = 
# 262 "./src/main/drivers/max7456.c" 3 4
                 1
# 262 "./src/main/drivers/max7456.c"
                     ;
}
...
{
    state.mutex = 
# 267 "./src/main/drivers/max7456.c" 3 4
                 0
# 267 "./src/main/drivers/max7456.c"
                      ;
}

static 
# 270 "./src/main/drivers/max7456.c" 3 4
      _Bool 
# 270 "./src/main/drivers/max7456.c"
           max7456TryLock(void)
{
    if (!state.mutex) {
        state.mutex = 
# 273 "./src/main/drivers/max7456.c" 3 4
                     1
# 273 "./src/main/drivers/max7456.c"
                         ;
        return 
# 274 "./src/main/drivers/max7456.c" 3 4
              1
# 274 "./src/main/drivers/max7456.c"
                  ;
    }
    return 
# 276 "./src/main/drivers/max7456.c" 3 4
          0
# 276 "./src/main/drivers/max7456.c"
               ;
}
...
uint16_t max7456GetScreenSize(void)
{
    ...
    if (state.isInitialized && (state.registers.vm0 & 13)) {
        return 390;
    }
    return 480;
}
...
static void max7456ReInit(void)
{
    ...
    if (state.dev == 
# 323 "./src/main/drivers/max7456.c" 3 4
                    ((void *)0)
# 323 "./src/main/drivers/max7456.c"
                        ) {
        return;
    }
    ...
    switch (state.videoSystem) {
        ...
        case NTSC:
            ...
            break;
        default:
            busRead(state.dev, 0xA0, &statVal);
            if ((!((statVal) & 0x04) && ((statVal) & 0x01))) {
                vm0Mode = 0x40;
            } else if ((!((statVal) & 0x04) && !((statVal) & 0x01))) {
                vm0Mode = 0x00;
            } else if ( millis() > 1500) {
    ...
    state.registers.vm0 = vm0Mode | 0x08;


    bufPtr = max7456PrepareBuffer(buf, bufPtr, 0x00, state.registers.vm0);
    bufPtr = max7456PrepareBuffer(buf, bufPtr, 0x04, (1 << 2));


    busTransfer(state.dev, 
# 358 "./src/main/drivers/max7456.c" 3 4
                          ((void *)0)
# 358 "./src/main/drivers/max7456.c"
                              , buf, bufPtr);


    bitArraySetAll(screenIsDirty, sizeof(screenIsDirty));
    if (!state.isInitialized) {
        max7456RefreshAll();
        state.isInitialized = 
# 364 "./src/main/drivers/max7456.c" 3 4
                             1
# 364 "./src/main/drivers/max7456.c"
                                 ;
    }
}
...
void max7456Init(const videoSystem_e videoSystem)
{
    ...
    if (state.dev == 
# 375 "./src/main/drivers/max7456.c" 3 4
                    ((void *)0)
# 375 "./src/main/drivers/max7456.c"
                        ) {
        return;
    }
    ...
    for (uint_fast16_t ii = 0; ii < (sizeof(screenBuffer) / sizeof((screenBuffer)[0])); ii++) {
        screenBuffer[ii] = (((((uint16_t)0x20) << 8) | 0) | (0x20 > 255 ? (1 << 2) : 0));
    }


    timeMs_t timeout = millis() + 50;
    while(max7456ReadVM0(&state.registers.vm0) &&
        (state.registers.vm0 | 0x02) &&
        millis() < timeout);
    ...
    for (int ii = 0; ii < 16; ii++) {
        bufPtr = max7456PrepareBuffer(buf, bufPtr, 0x10 + ii, ((0x00 << 2) | 0x01));
    }
...
{
    for (uint_fast16_t ii = 0; ii < (sizeof(screenBuffer) / sizeof((screenBuffer)[0])); ii++) {
        if (screenBuffer[ii] != (((((uint16_t)0x20) << 8) | 0) | (0x20 > 255 ? (1 << 2) : 0))) {
            screenBuffer[ii] = (((((uint16_t)0x20) << 8) | 0) | (0x20 > 255 ? (1 << 2) : 0));
...
{
    unsigned pos = y * 30 + x;
    uint16_t val = (((((uint16_t)c) << 8) | mode) | (c > 255 ? (1 << 2) : 0));
    ...
    max7456ReadChar(uint8_t x, uint8_t y, uint16_t *c, uint8_t *mode)
{
    ...
    if (pos < (sizeof(screenBuffer) / sizeof((screenBuffer)[0]))) {
        ...
        *mode = (val & 0xFF);
        ...
        return 
# 444 "./src/main/drivers/max7456.c" 3 4
              1
# 444 "./src/main/drivers/max7456.c"
                  ;
    }
    return 
...
static 
# 469 "./src/main/drivers/max7456.c" 3 4
      _Bool 
# 469 "./src/main/drivers/max7456.c"
           max7456DrawScreenPartial(void)
{
    uint8_t spiBuff[10 * (7 * 2)];
    int bufPtr = 0;
    int pos;
    ...
    for (pos = 0, updatedCharCount = 0;;) {
        ...
        charMode = (screenBuffer[pos] & 0xFF);
        uint8_t chr = (screenBuffer[pos] >> 8);
        if (((charMode) & (1 << 2))) {
            if (!(state.registers.dmm & (1 << 6))) {
                state.registers.dmm |= (1 << 6);
                bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x04, state.registers.dmm);
            }

            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x05, ph | (1 << 1));
            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x06, pl);





            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x07, charMode << 2);

            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x05, ph);
            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x06, pl);
            bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x07, chr);

        } else {
            if ((state.registers.dmm & (1 << 6)) || (((1 << 3) | (1 << 4) | (1 << 5)) & state.registers.dmm) != charMode) {
                state.registers.dmm &= ~(1 << 6);
                state.registers.dmm = (state.registers.dmm & ~((1 << 3) | (1 << 4) | (1 << 5))) | charMode;
...
# 533 "./src/main/drivers/max7456.c"
                                  , spiBuff, bufPtr);
        return 
# 534 "./src/main/drivers/max7456.c" 3 4
              1
# 534 "./src/main/drivers/max7456.c"
                  ;
    }
    return 
# 536 "./src/main/drivers/max7456.c" 3 4
          0
# 536 "./src/main/drivers/max7456.c"
               ;
}
...
static void max7456StallCheck(void)
{
    ...
    if (state.videoSystem == VIDEO_SYSTEM_AUTO) {
        ...
        if ((nowMs - lastSigCheckMs) > 1000) {


            busRead(state.dev, 0xA0, &statReg);

            if (statReg & 0x04) {
                videoDetectTimeMs = 0;
            } else {
                if (((!((statReg) & 0x04) && ((statReg) & 0x01)) && (((state.registers.vm0) & 0x40) == 0x00))
                    || ((!((statReg) & 0x04) && !((statReg) & 0x01)) && (((state.registers.vm0) & 0x40) == 0x40))) {
                    if (videoDetectTimeMs) {
                        if (nowMs - videoDetectTimeMs > 100) {
                            max7456ReInit();
                        }
                    } else {
...
void max7456Update(void)
{
    ...
    if (state.dev == 
# 584 "./src/main/drivers/max7456.c" 3 4
                    ((void *)0)
# 584 "./src/main/drivers/max7456.c"
                        ) {
        return;
    }
...
void max7456RefreshAll(void)
{
    ...
    if (state.dev == 
# 606 "./src/main/drivers/max7456.c" 3 4
                    ((void *)0)
# 606 "./src/main/drivers/max7456.c"
                        ) {
        return;
    }
    ...
    if (max7456TryLock()) {
        ...
        busRead(state.dev, 0x04 | 0x80, &dmm);
        busWrite(state.dev, 0x04, state.registers.dmm | (1 << 2));
        ...
        bitArrayClrAll(screenIsDirty, sizeof(screenIsDirty));
        for (unsigned ii = 0; ii < (sizeof(screenBuffer) / sizeof((screenBuffer)[0])); ii++) {
            if (!(((screenBuffer[ii] >> 8) == 0x20 || (screenBuffer[ii] >> 8) == 0x00) && !(((screenBuffer[ii] & 0xFF)) & (1 << 2)))) {
                bitArraySet(screenIsDirty, ii);
            }
        }
...
# 649 "./src/main/drivers/max7456.c" 3 4
   _Bool 
# 649 "./src/main/drivers/max7456.c"
        enabled = max7456OSDSetEnabled(
# 649 "./src/main/drivers/max7456.c" 3 4
                                       0
# 649 "./src/main/drivers/max7456.c"
                                            );

    busWrite(state.dev, 0x09, char_address);
    if (char_address > 255) {



        uint8_t addr_h = char_address >> 8;
        busWrite(state.dev, 0x0a, addr_h << 6);
    }
    busWrite(state.dev, 0x08, 0x50);

    max7456WaitUntilNoBusy();

    for (unsigned ii = 0; ii < sizeof(chr->data); ii++) {
        busWrite(state.dev, 0x0a, ii);
        busRead(state.dev, 0xC0, &chr->data[ii]);
    }
...
void max7456WriteNvm(uint16_t char_address, const max7456Character_t *chr)
{
    ...
    if (state.dev == 
# 678 "./src/main/drivers/max7456.c" 3 4
                    ((void *)0)
# 678 "./src/main/drivers/max7456.c"
                        ) {
        return;
    }
    ...
    max7456OSDSetEnabled(
# 684 "./src/main/drivers/max7456.c" 3 4
                        0
# 684 "./src/main/drivers/max7456.c"
                             );

    bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x09, char_address & 0xFF);

    uint8_t or_val = 0;
    if (char_address > 255) {
# 700 "./src/main/drivers/max7456.c"
        uint8_t addr_h = char_address >> 8;
        or_val = addr_h << 6;
    }

    for (unsigned x = 0; x < sizeof(chr->data); x++) {
        bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x0a, x | or_val);
        bufPtr = max7456PrepareBuffer(spiBuff, bufPtr, 0x0b, chr->data[x]);
    }
    ...
    busTransfer(state.dev, 
# 712 "./src/main/drivers/max7456.c" 3 4
                          ((void *)0)
# 712 "./src/main/drivers/max7456.c"
                              , spiBuff, bufPtr);
